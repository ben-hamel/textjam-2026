...
use cpal::{
    ...
    Device, FromSample, HostId, OutputCallbackInfo, Sample, SampleFormat,
...
fn main() -> anyhow::Result<()> {
    ...
    let mut jack_host_id: Result<HostId, anyhow::Error> = Err(anyhow::anyhow!("host unavailable"));
    #[allow(unused_mut, unused_assignments)]
    let mut pulseaudio_host_id: Result<HostId, anyhow::Error> = Err(anyhow::anyhow!("host unavailable"));
    #[allow(unused_mut, unused_assignments)]
    let mut pipewire_host_id: Result<HostId, anyhow::Error> = Err(anyhow::anyhow!("host unavailable"));
    #[cfg(any(
        target_os = "linux",
        target_os = "dragonfly",
        target_os = "freebsd",
    ...
    let host = if opt.jack {
        jack_host_id
            .and_then(|id| cpal::host_from_id(id).map_err(|e| anyhow::anyhow!(e)))
