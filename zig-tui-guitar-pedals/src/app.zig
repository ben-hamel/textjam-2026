...
pub const App = struct {
    state: *audio.SharedState,
    selected: usize,

    pub fn init(state: *audio.SharedState) App {
    ...
    pub fn run(self: *App) void {
        ...
        while (true) {
            ...
            if (key == ' ') {
                const cur = self.state.bypassed.load(.acquire);
                self.state.bypassed.store(!cur, .release);
                continue;
            }
            ...
            if (key == 0x1b) {
                ...
                switch (seq) {
                    'D' => if (self.selected > 0) {
                        self.selected -= 1;
                    },
                    'C' => if (self.selected + 1 < KNOBS.len) {
                        self.selected += 1;
                    },
                    'A' => self.adjust(STEP),
                    'B' => self.adjust(-STEP),
    ...
    fn adjust(self: *App, delta: f32) void {
        const ptr = switch (self.selected) {
            0 => &self.state.gain,
            1 => &self.state.tone,
    ...
    fn draw(self: *App, cols: u16, rows: u16) void {
        const gain = self.state.gain.load(.acquire);
        const tone = self.state.tone.load(.acquire);
        const volume = self.state.volume.load(.acquire);
        const bypassed = self.state.bypassed.load(.acquire);
        ...
        for (KNOBS, 0..) |name, i| {
            ...
            moveTo(ky, label_col);
            if (i == self.selected) write("\x1b[33;1m") else write("\x1b[0m");
            write(name);
            write("\x1b[0m");

            moveTo(ky + 1, kx);
            gauge(knob_values[i], knob_w, i == self.selected);
